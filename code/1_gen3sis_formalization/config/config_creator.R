...
# is present.
templatef  <- readLines("code/1_gen3sis_formalization/config/config_template_M2.R") # read template function to get the apply_trade_off function
...
# # the input variables and the simulations... so it;s easier to see if a mistake 
# # is present.
# templatef  <- readLines("code/1_gen3sis_formalization/config/config_template_M3.R") # read template function to get the apply_trade_off function
# function_line <- grep("^apply_trs_tradeoff <- function", templatef)
# # Find the end of the function declaration
# brace_count <- 0
# for (i in function_line:length(templatef)) {
...
# eval(function_expr) # load the apply_trs_tradeoff function!
# # end 
# permuts <- apply_trs_tradeoff(cbind(permuts, "temp_width"=0.4)) # apply it
# plot(permuts$dispersal, permuts$competition)
# permuts <- as.list(permuts)
# parameters <- permuts[1:length(parameters)]
# 
