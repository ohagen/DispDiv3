...
create_ancestor_species <- function(landscape, config) {
  ...
  for(i in unique(landscape$environment[,"patch"])){
    ...
    new_species[[i]]$traits[ , "competition"] <- comb_vector$competition
...
get_dispersal_values <- function(n, species, landscape, config) {
  # hist(rweibull(117546,shape=2, scale=50)) # sample of # of drawn approx max per time step.
...
divergence_threshold = comb_vector$divergence_threshold # e.g. 65 = 0.65 Myrs 650 kyrs
