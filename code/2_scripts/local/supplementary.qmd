...
stats_names <- c("gamma", "mtx.beta_prop_T","mtx.mean_alpha_T", "mtx.zeta_T","mtx.eta_T","maxlik_betasplit_.TF", "speciations_perc", "extinctions_perc")#
plot_stat_classes_summary(mbt, stats_names)
...
# remove finished
lmodmbt <- lapply(lmodmbt, function(x){
  x <- x[, -grep(c("trs.|_A|_B|_C|_D"),colnames(x))]
...
p.mat <- cor.mtest(M)
corrplot(M, method = "color", type =  "upper", diag=FALSE, mar=c(0,0,1,0),
          title="", 
...
stats_names <- c("gamma", "mtx.beta_prop_T","mtx.mean_alpha_T", "mtx.zeta_T","mtx.eta_T","maxlik_betasplit_.1", "speciations_perc", "extinctions_perc")#
plot_stat_classes_summary(mbt, stats_names)
```
...
lapply(lt, function(x){
  ita <<-ita+1
plot(x$trs.,x[[stats[stats_i]]], xlab=stats[1], ylab=stats[stats_i[indexi]], cex=0.5, xlim=c(0,1), ylim=ylimt)
...
stats <- grep("trs.*_50%", names(sss$M0$stats$t), value = T)
#stats <- grep("mean", stats, value = T)
# stats <- gsub("range_spatial_sps_", "",stats)
...
#| fig-height: 6
#| fig-cap: |
#|   The theory of limiting similarity predicts that as species diversity increases, the average population size of each species decreases due to the need for coexisting species to be sufficiently different to coexist, resulting in lower population sizes when higher competition is present.
