...
cor_stats_namess2 <- c(
  ...
  #"trs.spread_dispersal",
  #"trs.competition_50%",
  #"trs_competition_slope_time",
  #"trs.spread_competition",
  #"trs_temp_width_slope_time",
  #"trs.spread_temp_width",
  "speciations_perc",
  "extinctions_perc",
  #"turnover",
  "gamma",
...
# remove finished
# lmodmbt <- lapply(lmodmbt, function(x){
#   x <- x[, -grep(c("trs.|_A|_B|_C|_D|mtx.NA"),colnames(x))]
...
for (stats_i in 1:length(stats)){
  ...
  xlimt <- range(unlist(lapply(lt, function(x){x[["trs.dispersal_50%"]]})), na.rm=T)
  
lapply(lt, function(x){
  ita <<-ita+1
  
# plot(x$`trs.dispersal_50%`,x[[stats[stats_i]]], xlab="dispersal", ylab=stats[stats_i], cex=0.7, xlim=c(0,1), ylim=ylimt)
  y_n <- stats[stats_i]
  x_n <- "trs.dispersal_50%"
